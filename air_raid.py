...
def format_alert_message(region_name: str, alert_type: str = None) -> str:
    ...
    return f"🚨 УВАГА! Повітряна тривога в **{region_name}**!{alert_type_str}\nПрямуйте до укриття!"
...
async def check_air_raid_status(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    last_status = {region['regionId']: region for region in bot_data['data']}  # regionId как ключ (строка)
    ...
    for user_id, region_id in subscribers:
        # Убеждаемся, что region_id — строка или None
    ...
    for user_id, regions in subscribers_dict.items():
        try:
            # Если подписка на все регионы (None), обрабатываем все
